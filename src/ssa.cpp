...
bool SSAEvaluator::createVariable(uint32_t index, int32_t intBits, int32_t fracBits)
{
    // check if it already exists
    if (m_variables.find(index) == m_variables.end())
    {
        m_variables[index] = fplib::SFix(intBits, fracBits);
        return true;
    }
    else
    {
...
void SSAEvaluator::setVariable(uint32_t index, const fplib::SFix &value)
{
